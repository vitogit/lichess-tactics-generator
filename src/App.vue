    ...
    <section class="hero is-fullheight">
      ...
      <div class="hero-body">
        <div class="container has-text-centered">
          ...
          Enter your lichess username to generate your tactics.
...
export default {
  ...
  methods: {
    generate() {
      ...
      this.pullGames()
      // this.games = this.pullGames()
      
    },
    pullGames() {
      let url = `https://lichess.org/games/export/${this.username}?max=2&tags=true&analysed=true&evals=true&moves=true&perfType=blitz,rapid,classical`
      
      // this.$http.get(url, {headers: {'Accept': 'application/x-ndjson'}}).then(result => {
      //     console.log("result________",result.bodyText)
      //     this.games = result.bodyText.split("\n").filter(x => x !== "").map(x => JSON.parse(x))
      //     console.log("this.games________",this.games)
      // }, error => {
      //     console.error(error);
      // });      
      
      this.games = this.bodyText.split("\n").filter(x => x !== "").map(x => JSON.parse(x))
      console.log("this.games________",this.games)
      let blunders = []

      for (let game of this.games) {
        // console.log("game.analysis.filter(x => x.judgment)________",game.analysis.filter(x => x.judgment))
        // let blunders = game.analysis
        // .filter(x => x.judgment)
        // .filter(x => x.judgment.name === 'Blunder')
        
        // console.log('analysis:'+game.id)
        // console.log("blunders________",blunders)
        let lastEval = 0
        for (let [index, move] of game.analysis.entries() ) {
          // let whiteTurn = ((index+1) % 2) != 0 // Turn number odd then is white
          // Ignore blunder if white and already lost eval< -300
          // Ingore blunder if it keeps the last eval white and winning lastEval - eval > 
